...
{	
	...
	double diff[Dim] = {DC, DC};
	double x_max[Dim], x_min[Dim];
	...
	for (filenum = 1; filenum <= 47 ;filenum++) 
	{   
	    tau = 0;   
	   // tau2=50000; 
		printf("filenum:%d\n", filenum);  
	a1=0.1+ (filenum-1.0)*0.1;  
	       	x2[0] = 0.8035;x2[1] = 1.03081;//初始态 ，坐在N2占优势的态 
	       	x1[0] = 1.74095;x1[1] =0.0147918; //s取值较大的状态
			   
			  // x1[0]=0.0120118;x1[1]=1.50987;//s取值小的细胞状态 
		...
		fprintf(fp1, "%0.5f	%f	%f	%f	%f	%f	%f\n", a1, tau, var_t, xi[0], xi[1], xf[0],xf[1]);
	}                                                                                                                               
	fclose(fp1);
	fclose(fp2);
}
