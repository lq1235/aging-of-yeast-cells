...
main()
{
	...
	double fvec[Dim]={0},x[Dim]={1.55,0.02},xc[2],xi[2],xf[2];
	...
	double iter=1.0,Tn=1.0e9,Tni=1.0e6,Tnf,n_write=1.0e4;
	long   r=1,*point;
...
for(filenum=1;filenum<=180;filenum++)
{
	a1=0.01+ (filenum-1.0)*0.03; 
//	a1=0.125; 
...
double D1=a1, gamma2 = 1;

total_S=1.0*(a_St *(pow(K_St, 3)/ (pow(K_St, 3) + pow(D1, 3) ))- k_St * D1 + b_St);
